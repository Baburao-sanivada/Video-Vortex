...
const VideoContainer = () => {
  ...
  const getVideos = async () => {
    // console.log("Calling home page Recommendations api");
    const data = await fetch(youtube_video_api);
    // console.log("Home Page Recommendations Data :" + data.results);
    const json = await data.json();
    // console.log("Home Page Recommendations :" + json.response);
    var updatedData = videosList.concat(json.response.items);
      ...
      {videosList.length == 0 ? (
        ...
        videosList.map((video, index) => (
          <Link
            ...
            onClick={() => {
              dispatcher(setChannelId(video?.snippet?.channelId));
            }}
          >
