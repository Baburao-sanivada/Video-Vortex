...
const WpageVideoDetails = ({ videoId }) => {
  ...
  const fetchData = async () => {
    const data = await fetch(videoDetailsApi + videoId);
    const json = await data.json();
    setvideoDetails(json?.response.items[0]);
  };
    ...
    <div className="flex flex-col w-full">
      ...
      <div className="bg-gray-100 p-2 rounded-xl px-4 dark:bg-slate-600 ">
        ...
        <div className="lg:flex">
          ...
          <p className="text-blue-800 dark:text-blue-500">#{tags[0]}</p>
          {tags != null && tags.length > 1} &&{" "}
          <p className="text-blue-800 dark:text-blue-500">#{tags[1]}</p>
          {tags != null && tags.length > 2} &&{" "}
          <p className="text-blue-800 dark:text-blue-500">#{tags[2]}</p>
        </div>
